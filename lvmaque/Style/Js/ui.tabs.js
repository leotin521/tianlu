	...
	$.widget("ui.tabs", {
		...
		tabify: function(init) {
			...
			if (init) {
				...
				//打开页面时的ajax加载
				try{
					var Csx = $(this.$tabs[o.selected]);
					var Cgeturl = Csx.attr('ajax_href');
					var Chasget = Csx.attr('get')||0;
					var Cnowurl = Csx.attr('href');
					var Cvid = Cnowurl.split("#");
					if(Chasget!=1) setTimeout(function(){ajaxGetData(Cgeturl,Cvid[1])},500);//要延迟半秒，不然ie里不会显示ajax加载的内容
					Csx.attr('get','1');
				}catch(ex){
