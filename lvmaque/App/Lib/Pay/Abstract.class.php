...
abstract class paymentabstract {
	...
	{
		if (strtoupper($this->config['gateway_method']) == 'POST') $str = '<form action="' . $this->config['gateway_url'] . '" method="POST" target="_self" name="payform">';
		else $str = '<form action="' . $this->config['gateway_url'] . '" method="GET" target="_self" name="payform">';
	...
	protected function get_verify($url,$time_out = "60", $noheader = 2) {
        ...
        if(!$fp) {
            ...
            fclose($fp);
			if($noheader==0) return $info[1];
			elseif($noheader==1) return $res;
            elseif($noheader==2) {
				$info = implode(",",$info);
				return $info;
			}
