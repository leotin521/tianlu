...
// 管理用户模型
class ContractModel extends ACommonModel {
	protected $tableName = 'article_category'; 
	protected $_validate	=	array(
			array('type_name','require',"合同名称不能为空"),
			array('type_nid','require',"合同唯一标志不能为空"),
			array('type_nid','checkNid',"在此同级分类已有相同nid,请重新设置nid",0,'callback',self::MODEL_BOTH),
			array('type_nid','checkNids',"合同的唯一ID为系统路径有重复，会导致程序出错",0,'callback',self::MODEL_BOTH),
		);
