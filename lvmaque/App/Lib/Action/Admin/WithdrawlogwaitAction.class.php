    ...
    */
    ...
    public function edit() {
	 ...
	 public function doEdit() {
		...
		$status = htmlspecialchars(intval($_POST['withdraw_status']));
		$id = htmlspecialchars(intval($_POST['id']));
		$deal_info = htmlspecialchars($_POST['deal_info']);
		
		$secondfee = htmlspecialchars(floatval($_POST['withdraw_fee']));
		...
		if($vo['withdraw_status']<>3 && $status==3){
			...
			memberMoneyLog($vo['uid'],12,$vo['withdraw_money'],"提现未通过，返还提现金额：".$vo['withdraw_money']."元",'0','@网站管理员@',0,null,0,$id);
			...
			if( $vo['success_money'] < $vo['withdraw_money'] ){
			    ...
			    memberMoneyLog($vo['uid'],29,-($vo['withdraw_money']),"提现成功,扣除实际手续费".$vo['second_fee']."元，减去冻结资金，到账金额".($vo['withdraw_money']),'0','@网站管理员@');
				//$model->success_money = $vo['withdraw_money'];
			...
			if($vo['all_money']  <=$secondfee ){
				memberMoneyLog($vo['uid'],36,-($vo['withdraw_money']),"提现申请已通过，扣除实际手续费".$secondfee."元，到账金额".($vo['withdraw_money']-$secondfee)."元",'0','@网站管理员@',-$secondfee);
