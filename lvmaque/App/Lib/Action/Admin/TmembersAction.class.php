    ...
    */
    ...
    {	
		...
		if($newid){
			...
			alogs("Tmembers",$newid,1,"成功执行了".MembersModel::get_user_type(1)."的添加操作！");//管理员操作日志
			$this->success("添加成功",__URL__."/index");
		}else{
			logs("Tmembers",$newid,0,"执行".MembersModel::get_user_type(1)."的添加操作失败！");//管理员操作日志
			$this->error("添加失败，请重试");
		}
    }
	...
	public function doEdit() {
        ...
        if ($result&&$result2) { //保存成功
			$model->commit();
			alogs("Tmembers",0,1,"成功执行了".MembersModel::get_user_type(1)."的编辑操作！");//管理员操作日志
          //成功提示
            $this->assign('jumpUrl', __URL__);
            $this->success(L('修改成功'));
        } else {
			alogs("Tmembers",0,0,"执行".MembersModel::get_user_type(1)."的编辑操作失败！");//管理员操作日志
			$model->rollback() ;
            //失败提示
            $this->error(L('修改失败'));
        }
