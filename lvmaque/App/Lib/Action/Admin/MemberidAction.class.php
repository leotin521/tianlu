...
class MemberidAction extends ACommonAction
{
    ...
    */
	...
	public function doEdit(){
		...
		$uid = intval($_POST['id']);
        $deal_info = $this->_post('deal_info');
		//$credits = intval($_POST['deal_credits']);
        
        $newxid = setMemberStatus($uid, 'id', $status, 2, '实名', $db);
		if($status==1){
			$data['status']=1;
		...
		if($newxid){
            ...
            if($status==1){
                addInnerMsg($uid,"您的实名认证申请审核通过","您的实名认证申请审核通过");//站内信
                addCoupon($uid, 3, "实名认证奖励");    
            }else{
                addInnerMsg($uid,"您的实名认证申请审核未通过","您的实名认证申请审核未通过");//站内信
            }
