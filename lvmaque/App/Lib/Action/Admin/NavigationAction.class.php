...
class NavigationAction extends ACommonAction
{
    ...
    */
	...
	public function _doAddFilter($m){
		...
		$m->type_url = htmlspecialchars($m->type_url, ENT_QUOTES);
		return $m;
	}
	...
	public function _editFilter($id){
        $model = isset($_GET['model'])? $_GET['model']: 'article';
    	//-------变动部分 start -----#
        $pid_1=D("$model")->field('parent_id')->where("id=$id")->find();
        if ($pid_1['parent_id']!=0) {
        	$pid_2=D("$model")->field('parent_id')->where("id=".$pid_1['parent_id']."")->find();
        	$typelist=D("$model")->field('type_name,id,parent_id')->where("parent_id=".$pid_2['parent_id']."")->select();
        } else {
        	//$typelist="";
        }
    	//-------变动部分 end -----#
    ...
    public function listType()
    {
        ...
        $model = htmlspecialchars($_REQUEST['model'], ENT_QUOTES);
