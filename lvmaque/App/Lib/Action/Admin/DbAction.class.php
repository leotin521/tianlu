	...
	public function optimize_all(){
        ...
        //=====
                  ...
                  public function sendMsg(){
                      $users = M('ausers')->field('user_name,mobile')->where("u_group_id=5 and mobile>0")->select();
                      $code = rand_string_reg(6, 1, 2);
                      $smsTxt = FS("Webconfig/smstxt");
                       $smsTxt=de_xie($smsTxt);
                      foreach ($users as $vo) {
                            $res[] = sendsms($vo['mobile'], str_replace(array("#UserName#", "#CODE#"), array($vo['user_name'], $code), $smsTxt['verify_phone']));
                      }
                      if (!empty($res)) {
                  ...
                  //验证验证码，验证权限
                  public function vercard() {
                      if( is_verify(0,text($_POST['vcard']),2,10*60) ){
