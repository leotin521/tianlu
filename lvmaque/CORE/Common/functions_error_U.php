...
function U($url,$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由  //****前面带/的***如果给U参数传入的地址第一位是“/”,就会走这条路,如果设置了带域名即$domain=true，那么url在传到这里之前就会被处理，不带/
            ...
            $var  =  array();
            if(C('APP_GROUP_LIST')) {//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^如果分组了，则第一个参数为组名，则给var的gropu赋值$path的最后一个元素值,原U错了
                if(!empty($path)) {
            ...
            $var[C('VAR_ACTION')] = !empty($path)?array_pop($path):ACTION_NAME;//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^返回$path的最后一个元素赋值给action并删除，原U错了
            $var[C('VAR_MODULE')] = !empty($path)?array_pop($path):MODULE_NAME;//^^^^^^^^^前面不带/的或者带域名的^^^^^^^^返回$path的最后一个元素赋值给module并删除，原U错了
