...
{ 
	...
	{
        if( empty($relay_host) && empty($user) ) { // 未填选择默认的
            $msgconfig = FS("Webconfig/msgconfig");
            $smtp_port =$msgconfig['stmp']['port'];//25;
            $relay_host=$msgconfig['stmp']['server'];
            $auth = true;
            $user = $msgconfig['stmp']['user'];
            $pass = $msgconfig['stmp']['pass'];
        }
        $this->debug = FALSE;
        $this->smtp_port = $smtp_port;
        $this->relay_host = $relay_host;
        $this->time_out = 300; //is used in fsockopen()
        #
        $this->auth = $auth;//auth
        $this->user = $user;
        $this->pass = $pass;
